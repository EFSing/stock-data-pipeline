...
当前持仓集合仍由 `自选清单.启用` 表达，证券身份和数据源映射仍由同一行表达。`历史行情_未复权`、`历史行情_前复权`、`校验记录` 与 `运行日志` 是独立的历史/审计事实；CLOSE 只停用当前视图，不物理删除任何历史数据。未知的 `自选清单` 列保持原样，不被解释或覆盖。
...
| 操作 | 已有身份 | 历史行为 | 启用语义 |
|---|---|---|---|
| `ADD` | 已启用时幂等；已停用时要求 `REENTER` | 新身份初始化一年；已启用重复不抓 | 双历史成功后设为启用 |
