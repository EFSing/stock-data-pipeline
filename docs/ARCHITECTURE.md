...
- provider readiness 成功状态为 `IBKR_US_PROVIDER_READINESS_FROZEN`；对应 freeze artifact 状态为 `IBKR_PROVIDER_READINESS_FROZEN_NOT_ACQUIRED`。provider/API 不可用为 `US_PROVIDER_NOT_READY`，连接缺失为 `IBKR_CONNECTION_NOT_READY`，版本证据缺失为 `IBKR_VERSION_NOT_PROVEN`。未能证明 TWS/IBG、官方 client、server 或 application/build version 时 fail closed；未生成任何正式 validation OHLCV 或 B1 dataset。真实 capture 完成后，manifest 必须由 immutable version→canonical SHA-256 contract 锁定；在本地 TWS/IBG 未准备好时不伪造 manifest/hash。
