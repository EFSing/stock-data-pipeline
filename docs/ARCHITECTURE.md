...
- protocol 记录父级 Phase 5I `freeze_version=SETUP_03-FREEZE-2026-08-26-v1` 及其 `critical_values_sha256=sha256:447b20182f54b8c994042227bbfbaf94c50b2a9b4ade7332058a014915390a15`；SHA-256 canonical contract 覆盖除 integrity envelope 外的全部协议字段，因此 production candidates、诊断边界、市场、dataset selection、thresholds、selection rule、forbidden metrics、OOS 禁令或 parent identity 在同一 protocol version 下漂移都会失败
...
- Phase 5K 结构门槛固定为每市场每候选至少 8 个 CONFIRMED、相邻候选 Jaccard ≥60%、retention ≥80%、匹配日期漂移 median ≤5/P90 ≤15 个交易日、市场事件集中度 ≤35%、标的事件集中度 ≤25%、相邻候选每千 bar CONFIRMED 发生率增幅 ≤50%；样本不足与门槛失败分别使用预注册状态，不得根据结果调整门槛
- 正式选择规则为 lexicographic conservative：3% 全部通过则选 3%，否则 4%，否则 5%；全部不满足为 `VALIDATION_FAIL_NOT_READY_FOR_FORMAL_FREEZE`，主要因样本不足为 `INSUFFICIENT_VALIDATION_EVIDENCE`。禁止使用 forward return、MFE、MAE、win rate、P&L 或任何收益指标，不生成 market/regime-specific production 参数
