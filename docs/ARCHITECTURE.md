...
- Phase 5K-B1-A 只做 IBKR TWS/IB Gateway preflight、US contract identity resolution 与 `reqHeadTimeStamp` capability probe；live adapter 只从 `IBKR_TWS_API_PYTHON_PATH` 指定的 IBKR 官方 TWS API Python distribution lazy import `ibapi`，不在 `requirements.txt` 安装 PyPI `ibapi`，也不使用第三方 wrapper；不进入生产 `providers.py`，不调用 historical-bar API、HiThink、Replay、Trading Core、SETUP_03、Decision 或 Sheets。
- official Python client install contract：操作者必须从 IBKR 官方 TWS API distribution 安装/部署 Python client，设置 `IBKR_TWS_API_PYTHON_PATH` 指向包含 `ibapi/` 的官方 client root，并设置 `IBKR_API_PROVENANCE_FILE` 指向非敏感 JSON evidence。evidence 必须记录 `provider=Interactive Brokers`、`source_class=IBKR_OFFICIAL_TWS_API_DISTRIBUTION`、实际 `package_name/package_version`、client source-tree SHA-256、官方 source reference 与记录时间；运行时重新计算 source-tree hash，路径外的 `ibapi` 或 package/version/hash 不一致均 fail closed。
...
- API server version 只读取官方 client 的 `serverVersion()`；TWS/IB Gateway application/build version 不从 `ibapi` undocumented attributes 推断，只读取 `IBKR_HOST_VERSION_EVIDENCE_FILE` 指定的非敏感 JSON evidence，必须有匹配的 TWS/IBG application、version、approved source class、source reference、记录时间和 evidence SHA-256。当前 API `EWrapper.error` callback 使用包含 `errorTime` 的签名，不能通过固定旧 API 版本规避兼容性。
- provider readiness 成功状态为 `IBKR_US_PROVIDER_READINESS_FROZEN`；对应 freeze artifact 状态为 `IBKR_PROVIDER_READINESS_FROZEN_NOT_ACQUIRED`。provider/API/transport 不可用为 `US_PROVIDER_NOT_READY`，连接缺失为 `IBKR_CONNECTION_NOT_READY`，版本证据缺失为 `IBKR_VERSION_NOT_PROVEN`。60 个 candidate 全部 attempted 且各自进入客观 terminal status（`UNIQUE_RESOLVED_ADJUSTED_LAST_READY`、`SYMBOL_NOT_RESOLVABLE`、`CONTRACT_IDENTITY_NOT_UNIQUE`、`ADJUSTED_LAST_UNAVAILABLE` 或 `NO_IBKR_PERMISSION`）即可完成 provider-level capture；candidate failure 不自动升级为 provider failure，B1-A 不做 PRIMARY/RESERVE replacement。`reqContractDetails`/`reqHeadTimeStamp` timeout、无 terminal response、connectivity loss 或 session failure 必须保持 provider-level fail closed，并且不污染 candidate identity snapshot。
- runner 明确分为 `--mode capture` 与 `--mode validate-existing`：`capture` 才连接真实 TWS/IBG、执行完整 60-symbol readiness capture 并写出 immutable candidate manifest；即使 provider-level capture 完成，也固定返回 `IBKR_PROVIDER_READINESS_FROZEN_NOT_ACQUIRED`，不调用 final acceptance。`validate-existing` 只读取已经保存的 manifest，通过 committed `PINNED_MANIFEST_SHA256_BY_VERSION` 调用 `validate_manifest()`/`accept_frozen_manifest()`；不读取 live connection 配置、不创建 session、不调用 `reqContractDetails`/`reqHeadTimeStamp`、不改写文件，只有完整 pin/hash/结构匹配才返回 `IBKR_US_PROVIDER_READINESS_FROZEN`。缺 pin、tamper、hash mismatch 均 fail closed。未生成任何正式 validation OHLCV 或 B1 dataset；在本地 TWS/IBG 未准备好时不伪造 manifest/hash。
