...
- HiThink CN API capability/provenance smoke test（本独立分支）：2026-08-27 18:16:12 +08:00 在标准与提升权限执行上下文重跑代码表、CSI300/500/1000 成分、market-dump 签名端点、复权因子和交易日历；runner 未继承 `HITHINK_FINANCE_API_KEY`，7/7 HTTP 200 但 `code=2003`，原始响应已保存并哈希，当前不能证明真实数据权限或 as-of 能力，等待新进程/会话传播环境变量。
