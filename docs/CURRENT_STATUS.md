...
- Phase 5G Platform Tolerance Sensitivity Study（开发中，基于 PR #17 的堆叠分支）：锁定同一 9 标的/6032 bars frozen dataset 与 production 参数版本，仅按固定顺序改变 `platform_tolerance_pct`（0% 至 10% 共 9 档）；每档复用 strict as-of Replay、同源 Setup/Decision diagnostics 与 T+1 execution contract，输出完整漏斗、reason 守恒、标的/市场/年份分布、5/10/20D forward return/MFE/MAE 和结构稳定性。不排名、不选最佳参数、不写回生产配置、不启动 OOS。
