...
- Phase 5F SETUP_03 Confirmation Gate Diagnostics（开发分支 `research/setup03-confirmation-gate-diagnostics`）：production Setup 同一次计算返回原 `Setup` 与只读 `SetupDiagnostics`，兼容 API 和交易行为不变；Replay 只携带 diagnostics，research 仅投影互斥守恒 terminal reason、逐 gate 漏斗、辅助多重失败与 near-miss 分布。固定集本地同源计算初步守恒为 6032 bars；不改生产参数/规则，不优化，不启动 OOS。待 PR CI 与真实 frozen workflow 后回填最终 run 证据。
