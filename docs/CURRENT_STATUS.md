...
- Phase 5J SETUP_03 Structural Validation Protocol Freeze（独立分支，待 PR）：新增版本化机器可读 `setup03_structural_validation_protocol.json` 与只读 loader/static AST audit，父级锁定 Phase 5I `SETUP_03-FREEZE-2026-08-26-v1` / `sha256:447b20182f54b8c994042227bbfbaf94c50b2a9b4ade7332058a014915390a15`；protocol integrity 为 `sha256:2109b7aa9f9245e2772131766efc75ee4bb747d46e36573fb609271696d0e7f8`。正式候选仅 3.0%/4.0%/5.0%，2.5%/5.5%/7.5%/10.0% 仅诊断边界；lookback=5、window=40 仅保留 incumbent design constant，不声明最优，market/regime/波动率 production 规则禁用。已预注册 CN/HK/US/JP/SE、每市场≥8/总计≥40 标的及每市场约 6000 有效日 K bars 的 development-validation manifest 规则、结构门槛和 lexicographic conservative selection rule，但本阶段不执行 validation、不选择正式参数、不抓取数据、不接触最终 OOS。最终状态：`VALIDATION_PROTOCOL_REGISTERED_NOT_EXECUTED`。
