...
- Phase 5K-A SETUP_03 Development-Validation Symbol Manifest Freeze（本 PR）：新增 metadata-only `research/development_validation_symbol_manifest.py` 与 frozen `development_validation_symbol_manifest.json`；固定 CN/HK/US/JP/SE，每市场 14 个（10 PRIMARY + 4 RESERVE），未来 active hard minimum 为每市场 8/全市场 40，broad-sector cap 为 2。manifest version 为 `SETUP_03-DEVELOPMENT-VALIDATION-SYMBOL-MANIFEST-2026-08-27-v1`，状态为 `MANIFEST_FROZEN_NOT_FETCHED`，canonical SHA-256 由不可变 version/hash contract 保护；本阶段未读取 validation OHLCV、SETUP_03 输出、收益指标或最终 OOS。
