...
A股的`AKShare`或`BaoStock`重试失败，或者虽然成功返回但最新日期落后于收盘后的目标交易日时，程序会将该来源判定为失效，并使用腾讯与新浪实时行情接口补充最新未复权快照。腾讯不可用时会继续尝试新浪；两者不提供历史日线及前复权序列，因此只作为当日收盘应急源。若主源与校验源最终落到同一个快照接口，程序会标记为“单源可用”，不会误判为双源验证成功。
