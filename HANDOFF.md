...
`ARMED_OPPORTUNITY_PROJECTION_V1` 已完成小范围 presentation/read-only 实现与本地验证。
正式节点：`READY_FOR_UI_PRODUCT_ACCEPTANCE_ARMED_OPPORTUNITY_PROJECTION_V1`。
下一步只做独立 PR 的 CI 与 UI/产品验收，不扩展新的策略研究或生产状态。
...
- expected Entry Zone 是“达到确认后按现有规则可观察的区域”，不是当前 plan、买入信号或未来成交承诺。
