...
`CLOUD_DAILY_REPORT_PRODUCT_SEMANTICS_V1` 正在独立分支
`fix/cloud-report-product-semantics-v1` 上完成最小 presentation/read-only 修正；准备创建
独立 PR，不自动 merge。范围仅为 Cloud Daily Report 的等待确认、确认日 NO_TRADE 首层摘要、
策略跟踪持仓／模拟持仓标签，不改变交易策略或任何持久化语义。

## Current State

- 当前分支：`fix/cloud-report-product-semantics-v1`；PR 尚未创建、未合并。基线 main 已包含 PR #93 的 ARMED projection。
...
提交当前最小改动并推送分支，创建独立 PR（不自动 merge），等待 CI 与 UI/product acceptance；
之后再回到真实 prospective Cloud Daily Reports / Paper 数据观察，不自动启动新的 strategy
